...
if ($kd_matkul) {
	if ($kd_jurusan&&$semester&&$lama&&$kd_ruang&&$hari) {		
		...
		//cek waktu,hari,ruangan		
		$sqlwaktu = "SELECT * FROM tb_jadwal,tb_matkul WHERE tb_matkul.kd_matkul=tb_jadwal.kd_matkul 
		...
		if ($kt>0){
			...
			$sh = $ck['kd_ruang'];
			$s = $ck['hari'];
			echo "<script>alert('Jadwal bentrok dengan jadwal Matakuliah $show di ruangan $sh pada hari $s ');window.history.back()</script>";
   		}else{
   			//cek matkul, rungan
   			...
   			if ($ktmat>0){
				...
				echo "<script>alert('Matakuliah sudah diambil oleh kelas ini ($kelas) !');window.history.back()</script>";
	   			...
	   			if ($ktdos>0){
					...
					if($simpan){
							echo "<script>alert('Data Berhasil Ditambahkan !');window.location='../../index.php?page=dt_jadwal';</script>";
					}else{
