...
<div class="card mb-3">
  ...
  <div class="card-header">Hasil Pencarian Jadwal Berdasarkan (Semester/Hari) : <?php echo $smt; ?> / <?php echo $h; ?> 
      <a class="btn btn-danger btn-block col-md-3" href="index.php?page=export_data" style="font-size: 14px;float: right;margin-bottom: 0px;"><i class="fa fa-table"></i> View All Data to Export</a>
  </div>
    <div class="card-body">
...
<?php
  if (!empty($smt)) {
    //echo "<script>alert('smt ada');</script>";
    if (!empty($h)){
      //echo "<script>alert('smt dan h ada');</script>";
        $sqljur = "SELECT * FROM tb_jadwal,tb_matkul,tb_dosen,tb_jurusan WHERE 
          ...
          AND semester='$smt' AND hari='$h' order by waktu1 ASC";
      ...
      $sqljur = "SELECT * FROM tb_jadwal,tb_matkul,tb_dosen,tb_jurusan WHERE 
          ...
          AND tb_jurusan.kd_jurusan=tb_jadwal.kd_jurusan
          AND tb_matkul.kd_matkul=tb_jadwal.kd_matkul
          AND semester='$smt' order by kelas ASC";
        $queryjur = mysqli_query($koneksi,$sqljur);
        if(mysqli_num_rows($queryjur)==0){  
          //jika data kosong, maka akan menampilkan row kosong
          echo '<tr><td colspan="6">Tidak ada data!</td></tr>';
    ...
    if(!empty($smt)){
      //echo "<script>alert('h n smt');</script>";
      $sqljur = "SELECT * FROM tb_jadwal,tb_matkul,tb_dosen,tb_jurusan WHERE 
              tb_matkul.kd_matkul=tb_jadwal.kd_matkul 
          AND tb_dosen.id_dosen=tb_jadwal.kd_dosen 
          AND tb_jurusan.kd_jurusan=tb_jadwal.kd_jurusan
      ...
      //echo "<script>alert('h doang');</script>";
      $sqljur = "SELECT * FROM tb_jadwal,tb_matkul,tb_dosen,tb_jurusan WHERE 
              tb_matkul.kd_matkul=tb_jadwal.kd_matkul 
          AND tb_dosen.id_dosen=tb_jadwal.kd_dosen 
          AND tb_jurusan.kd_jurusan=tb_jadwal.kd_jurusan
          AND tb_matkul.kd_matkul=tb_jadwal.kd_matkul
          AND hari='$h' order by kd_ruang ASC";
        $queryjur = mysqli_query($koneksi,$sqljur);
        if(mysqli_num_rows($queryjur)==0){  
          //jika data kosong, maka akan menampilkan row kosong
          echo '<tr><td colspan="6">Tidak ada data!</td></tr>';
